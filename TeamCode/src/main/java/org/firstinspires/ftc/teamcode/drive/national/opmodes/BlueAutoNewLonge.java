...
/**
 * Autônomo Blue Longe (trás): trajetórias do BlueAutoNewLonge; intake/shooter ligados o tempo todo;
 * turret travada em -135°; em cada scorePose espera estabilizar, atira (flap) e segue.
 */
...
public class BlueAutoNewLonge extends OpMode {
    ...
    private boolean state3ScorePickup1Started = false;
    ...
    /** Hood fixo o autônomo todo na posição da zona de score (Red=0.7, igual TeleOp na zona). */
    private static final double AUTO_HOOD_POSITION_LONGE = 0.7;

    // Poses do autônomo de trás (Longe)
    ...
    private PathChain grabPickup1, endGrabPickup1, scorePickup1, grabPickup2, scorePickup2, end;
    ...
    public void setPathState(int pState) {
        ...
        if (pState == 3 || pState == 12) scorePhaseStartTime = 0;
        if (pState == 3) state3ScorePickup1Started = false;
    ...
    public void autonomousPathUpdate() {
        ...
        switch (pathState) {
            ...
            case 3:
                if (!state3ScorePickup1Started && !follower.isBusy()) {
                    follower.setMaxPower(AUTO_MAX_DRIVE_POWER);
                    follower.followPath(scorePickup1, 1, true);
                    state3ScorePickup1Started = true;
                    break;
                }
    ...
    public void init() {
        ...
        if (robot.turret != null) {
            ...
            robot.turret.lockAngle(ConstantsConf.Nacional.AUTO_TURRET_LOCKED_ANGLE_BLUE_LONGE_DEG);
        }
    ...
    public void loop() {
        ...
        if (robot.turret != null) robot.turret.lockAngle(ConstantsConf.Nacional.AUTO_TURRET_LOCKED_ANGLE_BLUE_LONGE_DEG);

        if (robot.hood != null && robot.hood.isEnabled()) robot.hood.setPositionOverride(AUTO_HOOD_POSITION_LONGE);
